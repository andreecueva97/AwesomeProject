...
const Juego_Mapa = ({ navigation, route }) => {
  ...
  const Xh = (Dimensions.get('window').height * 441) / 759.2727;// dimension de height del mapa  imagen
  const Xw = (Dimensions.get('window').width * 567) / 392.7272;// dimension de width del mapa  imagen
  const fontSizeX = (18 *Dimensions.get('window').height*Dimensions.get('window').width)/(759.2727*392.7272);
  return (
    <SafeAreaView style={{ flex: 1 }}>
      <View style={styles.inicio_View}>
        <View style={{ flex: 1, zIndex: 10, backgroundColor: 'grey', }}>
          {/* {console.log(Dimensions.get('window').height)} */}
          <Image style={[styles.imagenMapa, { height: Xh, width: Xw }]} source={require('@img/testmandados_LaCasaModificado.jpg')} />
          <>
            {images.map((img, index) =>
              <Image style={{
                top: (350 * Dimensions.get('window').height) / 759.2727, left: (195 * Dimensions.get('window').width) / 392.7272, position: 'absolute', opacity: img.opacity,
                height: Xh, width: Xw
              }} source={img.source} key={index} />
            )
            }
          </>
          <View style={[styles.buttonLocalidad, { top: (-40 * Dimensions.get('window').height) / 759.2727, left: (653 * Dimensions.get('window').width) / 392.7272, }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('zapatero'); }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center' }}>{marcarNumeroLocalidad(9)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: (-96 * Dimensions.get('window').height) / 759.2727, left: (527 * Dimensions.get('window').width) / 392.7272, }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('correo') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center' }}>{marcarNumeroLocalidad(10)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: (-73 * Dimensions.get('window').height) / 759.2727, left: (340 * Dimensions.get('window').width) / 392.7272, }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('cafe') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center' }}>{marcarNumeroLocalidad(2)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: (-62 * Dimensions.get('window').height) / 759.2727, left: (252 * Dimensions.get('window').width) / 392.7272, }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('almacen') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center' }}>{marcarNumeroLocalidad(1)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: (-69 * Dimensions.get('window').height) / 759.2727, left: (461 * Dimensions.get('window').width) / 392.7272, }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('oficina') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center' }}>{marcarNumeroLocalidad(5)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: (-78 * Dimensions.get('window').height) / 759.2727, left: (643 * Dimensions.get('window').width) / 392.7272, }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('estacion') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center' }}>{marcarNumeroLocalidad(8)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: (-44 * Dimensions.get('window').height) / 759.2727, left: (647 * Dimensions.get('window').width) / 392.7272, }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('kiosko') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center' }}>{marcarNumeroLocalidad(7)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: (-102 * Dimensions.get('window').height) / 759.2727, left: (520 * Dimensions.get('window').width) / 392.7272, }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('amiga') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center' }}>{marcarNumeroLocalidad(4)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: (-54 * Dimensions.get('window').height) / 759.2727, left: (568 * Dimensions.get('window').width) / 392.7272, }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('libreria') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center' }}>{marcarNumeroLocalidad(6)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: (-104 * Dimensions.get('window').height) / 759.2727, left: (379 * Dimensions.get('window').width) / 392.7272, }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('panaderia') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center' }}>{marcarNumeroLocalidad(3)}</Text>
            </TouchableOpacity>
          </View>
       ...
       width:100,height:100,
        ...
        <TouchableOpacity style={{ borderRadius: 30 / 2, height: 30, width: 30,backgroundColor:'blue' }} onPress={() => { funcionnOpacity2('panaderia') }}>
        ...
        <View style={{ zIndex: -10, top: (120 * Dimensions.get('window').height) / 759.2727, alignContent: 'center', justifyContent: 'center', flexDirection: 'row', borderRadius: 3, marginBottom: 10, left: (-280 * Dimensions.get('window').width) / 392.7272 }}>
        ...
        <View style={{ zIndex: -10, top: (120 * Dimensions.get('window').height) / 759.2727, alignContent: 'center', justifyContent: 'center', flexDirection: 'row', borderRadius: 3, marginBottom: 10, left: (-280 * Dimensions.get('window').width) / 392.7272 }}>
          <View style={{ justifyContent: 'center', alignItems: 'center', }}>
            <FlatList //lista de localidades en vista con sus nombres y ids y tiempo y posiciones de haber ido
              ...
              data={posiciones}
              style={{ fontSize: 0, color: 'white', height: (150*Dimensions.get('window').height)/759.2727, }}
              renderItem={({ item }) =>
                <View style={{ fontSize: fontSizeX, backgroundColor: '#DFDFE2', justifyContent: 'space-between', alignItems: "center", flexDirection: 'row', justifyContent: "center" }}>
                  {/* {console.log(item), console.log(juego.findIndex((element) => element === item))} */}
                  <View style={{ backgroundColor: 'white' }} >
                    <Text style={{ textAlign: 'center', justifyContent: "center", alignItems: "center", width: (40*Dimensions.get('window').width)/392.7272, fontSize: (15*Dimensions.get('window').height*Dimensions.get('window').width)/(759.2727*392.7272), color: '#371B1F', top: (4 * Dimensions.get('window').height) / 759.2727, backgroundColor: '#DFDFE2' }}>
                      {/* {(juegoPosiciones.findIndex((element) => element === item))} */}
                      {posiciones.findIndex(element => element === item)}
                    </Text>
                  </View>
                  <View style={{ backgroundColor: 'white' }} >
                    <Text style={{ textAlign: 'center', justifyContent: "center", alignItems: "center", width: (70*Dimensions.get('window').width)/392.7272, fontSize: (15*Dimensions.get('window').height*Dimensions.get('window').width)/(759.2727*392.7272), color: '#371B1F', top: (4 * Dimensions.get('window').height) / 759.2727, backgroundColor: '#DFDFE2' }}>
                      {item}
                    </Text>
                  </View>
                  <View style={{ backgroundColor: 'white' }} >
        ...
        <View style={{ zIndex: 10, top: (140 * Dimensions.get('window').height) / 759.2727, alignContent: 'center', justifyContent: 'center', flexDirection: 'row', borderRadius: 3, marginBottom: 10, left: (-277 * Dimensions.get('window').width) / 392.7272 }}>

          <TouchableOpacity style={[styles.inicio_Button, { left: (66 * Dimensions.get('window').width) / 392.7272, backgroundColor: '#AABECF' }]} onPress={() => {
            agregarJuego_User();
          }}>
            <Text style={styles.inicio_Text}>Terminar</Text>
          </TouchableOpacity>
          <View style={{
          zIndex: 30, top: (5 * Dimensions.get('window').height) / 759.2727,left: (120 * Dimensions.get('window').width) / 392.7272, alignContent: 'center', justifyContent: 'center', flexDirection: 'row', borderRadius: 3,
          marginBottom: 10,width:130
        }}>
          <TouchableOpacity
            style={[styles.inicio_Button, { borderColor: 'black',borderWidth: 1,width:130,backgroundColor: '#DFDFE2', fontSize: (17*Dimensions.get('window').height*Dimensions.get('window').width)/(759.2727*392.7272), top: (2 * Dimensions.get('window').height) / 759.2727,zIndex:20 }]} onPress={() => { setModalVisible(!modalVisible); }} >
            <Text style={{ fontSize: (14*Dimensions.get('window').height*Dimensions.get('window').width)/(759.2727*392.7272), alignContent: 'center', justifyContent: 'center', alignItems: 'center', textAlign: 'center', }}>
              Informacion
            </Text>
          </TouchableOpacity>
        </View>
        </View>
        <View style={[styles.RevisionGeneral_Button_Calle, { top: (41 * Dimensions.get('window').height) / 759.2727, zIndex: 10 }]}>
          {/* <TouchableOpacity 
          ...
          >
            <View style={{ zIndex: 10, left: 10, top: 300, transform: [{ rotate: '90 deg' }], backgroundColor: 'white' }}>
              <View style={{ backgroundColor: "#AABECF", padding: 20, borderRadius: 3 }}>
                ...
                >
                  <Text style={{ color: 'white', fontSize: (18*Dimensions.get('window').height*Dimensions.get('window').width)/(759.2727*392.7272) }}>continuar</Text>
                </TouchableHighlight>
              </View>
            </View>
          </Modal>
...
const styles = StyleSheet.create({
  circuleButton:{
     borderRadius: (30*Dimensions.get('window').height*Dimensions.get('window').width)/(759.2727*392.7272) / 2, height: (30*Dimensions.get('window').height*Dimensions.get('window').width)/(759.2727*392.7272), width: (30*Dimensions.get('window').height*Dimensions.get('window').width)/(759.2727*392.7272), 
  },
  imagenMapa: {
    top: (350 * Dimensions.get('window').height) / 759.2727, left: (195 * Dimensions.get('window').width) / 392.7272, opacity: 1,
    //  height:Dimensions.get('window').height-180,width:500 
  },
  buttonLocalidad: {//se refiere a el estilo de los botones de las localidades
    backgroundColor: '#B5B5BA',
    alignContent: 'center', justifyContent: 'center', flexDirection: 'row',
    borderRadius: (30*Dimensions.get('window').height*Dimensions.get('window').width)/(759.2727*392.7272) / 2, height: (30*Dimensions.get('window').height*Dimensions.get('window').width)/(759.2727*392.7272), width: (30*Dimensions.get('window').height*Dimensions.get('window').width)/(759.2727*392.7272), zIndex: 12,
  },
  RevisionGeneral_Button_Calle: {
    alignContent: 'center',
    flexDirection: 'row',
    transform: [{ rotate: '66 deg' }],
    height: (120*Dimensions.get('window').height)/759.2727, width: (10*Dimensions.get('window').width)/392.7272,
    left: (162 * Dimensions.get('window').width) / 392.7272,
  },
  inicio_View: {
  ...
  inicio_Text: {
    fontSize: (30*Dimensions.get('window').height*Dimensions.get('window').width)/(759.2727*392.7272),
    alignContent: 'center', justifyContent: 'center',
  ...
  inicio_Button: {
    ...
    width: (180*Dimensions.get('window').width)/392.7272,
