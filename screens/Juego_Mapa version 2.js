...
// lo que estaba antes funciona con el celu         <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  
...
const Juego_Mapa = ({ navigation, route }) => {
  ...
  const flatlistLocalidades = () => {
    if (Dimensions.get('window').width > 440) {
      return (
        <View style={{ width: '100%', zIndex: -10, top: topFlatlistD, alignContent: 'center', justifyContent: 'center', flexDirection: 'row', borderRadius: 3, marginBottom: 10, left: '-38%' }}>
          <View style={{ justifyContent: 'center', alignItems: 'center', width: '106%' }}>
            <FlatList //lista de localidades en vista con sus nombres y ids y tiempo y posiciones de haber ido
              //data={realm.objects('Juego')}
              data={posiciones}
              style={{ fontSize: 0, color: 'white', height: '20.2%' }}
              renderItem={({ item }) =>
                <View style={{ fontSize: fontSizeXX, backgroundColor: '#DFDFE2', justifyContent: 'space-between', alignItems: "center", flexDirection: 'row', justifyContent: "center" }}>
                  ...
                  <View style={{ backgroundColor: 'white' }} >
                    <Text style={{ textAlign: 'center', justifyContent: "center", alignItems: "center", width: Dimensions.get('window').width * 0.1, fontSize: width / 36, color: '#371B1F', top: '10%', backgroundColor: '#DFDFE2' }}>
                      {/* {(juegoPosiciones.findIndex((element) => element === item))} */}
                      {posiciones.findIndex(element => element === item)}{'  '}
                    </Text>
                  </View>
                  <View style={{ backgroundColor: 'white' }} >
                    <Text style={{ textAlign: 'center', justifyContent: "center", alignItems: "center", width: Dimensions.get('window').width * 0.19, fontSize: width / 36, color: '#371B1F', top: '10%', backgroundColor: '#DFDFE2' }}>
                      {item}
                    </Text>
                  </View>
                  <View style={{ backgroundColor: 'white' }} >
            ...
            <FlatList //lista de localidades en vista con sus nombres y ids y tiempo y posiciones de haber ido
              ...
              renderItem={({ item }) =>
                <View style={{ fontSize: fontSizeXX, backgroundColor: '#DFDFE2', justifyContent: 'space-between', alignItems: "center", flexDirection: 'row', justifyContent: "center" }}>
                  {/* {console.log(item), console.log(juego.findIndex((element) => element === item))} */}
                  <View style={{ backgroundColor: 'white' }} >
                    <Text style={{ textAlign: 'center', justifyContent: "center", alignItems: "center", width: Dimensions.get('window').width * 0.1, fontSize: width / 25, color: '#371B1F', top: '10%', backgroundColor: '#DFDFE2' }}>
                      {/* {(juegoPosiciones.findIndex((element) => element === item))} */}
                      {posiciones.findIndex(element => element === item)}{'  '}
                    </Text>
                  </View>
                  <View style={{ backgroundColor: 'white' }} >
                    <Text style={{ textAlign: 'center', justifyContent: "center", alignItems: "center", width: Dimensions.get('window').width * 0.19, fontSize: width / 25, color: '#371B1F', top: '10%', backgroundColor: '#DFDFE2' }}>
                      {item}
                    </Text>
                  </View>
                  <View style={{ backgroundColor: 'white' }} >
  ...
  const pantallaJuego = () => {
    if (Dimensions.get('window').width > 440) {
      return (
        <View style={{ height: heigthD, width: Xw, left: '-30%', top: '-84%' }}>
          {console.log(Dimensions.get('window').width)}
          <View style={[styles.buttonLocalidad, { top: '-1%', left: '186.3%', }]}>
            <TouchableOpacity style={[styles.circuleButton, {}]} onPress={() => { funcionnOpacity2('zapatero'); }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(9)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: '-15%', left: '158%' }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('correo') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(10)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: '-14%', left: '117%', }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('cafe') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(2)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: '-18%', left: '96%', }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('almacen') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(1)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: '-25%', left: '143.5%', }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('oficina') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(5)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: '-32%', left: '186%', }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('estacion') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(8)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: '-27%', left: '188%' }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('kiosko') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(7)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: '-45%', left: '157%' }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('amiga') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(4)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: '-43%', left: '170%' }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('libreria') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(6)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: '-55%', left: '126%' }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('panaderia') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(3)}</Text>
            </TouchableOpacity>
          </View>
        </View>
      );
      ...
      return (
        <View style={{ height: heigthD, width: Xw, left: '-22%', top: '-10%' }}>

          <View style={[styles.buttonLocalidad, { top: '-3%', left: '186.3%', }]}>
            <TouchableOpacity style={[styles.circuleButton, {}]} onPress={() => { funcionnOpacity2('zapatero'); }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(9)}</Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={[styles.buttonLocalidad, { top: '-13%', left: '96%', }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('almacen') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(1)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: '-16%', left: '143.5%', }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('oficina') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(5)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: '-18%', left: '184%', }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('estacion') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(8)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: '-13%', left: '186%' }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('kiosko') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(7)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: '-25%', left: '157%' }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('amiga') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(4)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: '-19%', left: '167%' }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('libreria') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(6)}</Text>
            </TouchableOpacity>
          </View>
          <View style={[styles.buttonLocalidad, { top: '-30%', left: '125%' }]}>
            <TouchableOpacity style={styles.circuleButton} onPress={() => { funcionnOpacity2('panaderia') }}>
              <Text style={{ fontSize: fontSizeX, textAlign: 'center', color: 'black' }}>{marcarNumeroLocalidad(3)}</Text>
            </TouchableOpacity>
          </View>
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      <View style={styles.inicio_View}>
        ...
        <View style={{ zIndex: -10, top: topClockD, alignContent: 'center', justifyContent: 'center', flexDirection: 'row', borderRadius: 3, marginBottom: 10, left: '-38%' }}>
          <Clock minuto={minutoMapa} segundo={segundoMapa} />
        </View>
        {flatlistLocalidades()}
        <View style={{ width: '101.5%', zIndex: 10, top: '18%', alignContent: 'center', justifyContent: 'center', flexDirection: 'row', borderRadius: 3, marginBottom: 10, left: '-37.7%' }}>

          <TouchableOpacity style={[styles.inicio_Button, { left: '37.7%', backgroundColor: '#AABECF', width: '100%' }]} onPress={() => {
            agregarJuego_User();
          }}>
            <Text style={styles.inicio_Text}>Terminar</Text>
          </TouchableOpacity>
          <View style={{
            zIndex: 30, top: '4%', left: '15%', alignContent: 'center', justifyContent: 'center', flexDirection: 'row', borderRadius: 3,
            marginBottom: 10, width: '18%'
          }}>
            <TouchableOpacity
              style={[styles.inicio_Button, { borderColor: 'black', borderWidth: 1, width: '100%', backgroundColor: '#DFDFE2', fontSize: width / 30, top: '1%', zIndex: 20 }]} onPress={() => { setModalVisible(!modalVisible); }} >
              <Text style={{ fontSize: width / 40, alignContent: 'center', justifyContent: 'center', alignItems: 'center', textAlign: 'center', }}>
                Informacion
              </Text>
            </TouchableOpacity>
          </View>
        </View>
        <View style={[styles.RevisionGeneral_Button_Calle, { top: '5.5%', zIndex: 10 }]}>
        </View>
        <View styles={{}}>
          <Modal
            animationType="none"
